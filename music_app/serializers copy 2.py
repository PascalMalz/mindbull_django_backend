...
class CompositionSerializer(serializers.ModelSerializer):
    ...
    def handle_composition_audio(self, ca_data, parent_composition):
        logger.debug("Handling composition audio: %s", ca_data)
        content_type = ca_data.pop('content_type', None)
        content_data = ca_data.pop('content', None)
        
        # Proceed only if content_data is not None
        if content_data is not None:
            # Existing logic to create audio instance
            if content_type == 'audio_file':
                ...
                # Handle composition type if needed
                pass
        else:
            logger.error("content_data is None for composition audio: %s", ca_data)
            # Handle the error appropriately, maybe raise a ValidationError
            raise ValidationError("Missing content data for composition audio.")
    ...
    def create(self, validated_data):
        ...
        logger.debug(f"composition_audios_data: {composition_audios_data}")
